...
if (!empty((isset($_POST["cancelar"])) ? $_POST["cancelar"] : "")) {
  $id = (isset($_POST["cancelar"])) ? $_POST["cancelar"] : "";
}
...
if (!empty((isset($_POST["guardar"])) ? $_POST["guardar"] : "")) {
  ...
  // //*$query=$conexion->prepare("CALL P_puntuacion($id);");
  // $query->execute();
  // $listaP=$query->fetchAll(PDO::FETCH_ASSOC);
  // $PU=$listaP[0]["promedio"];
  // if (!empty($PU)) {
  //   $query=$conexion->prepare("UPDATE `pelicula` SET `puntuacion`=$PU");
  //   $query->execute();
  //   $listaP=$query->fetchAll(PDO::FETCH_ASSOC);
  // }
  // else {
...
$pp = $listaP[0]["precio"];
$rh = $listaP[0]["rentadasH"];
$PU = $listaP[0]["puntuacion"]
...
<div class="col-md-9">
  ...
  <div class="row">
    ...
    <?php
    $query = $conexion->prepare("SELECT `F_disponibles`($id) AS `F_disponibles`;");
    $query->execute();
    $listaP = $query->fetchAll(PDO::FETCH_ASSOC);
    $dis = $listaP[0]["F_disponibles"];
    ?>
    <div class="col-md-4">
      </br>
      </br>
      ...
      <h2>Puntuacion : <strong>
          <?php echo $PU ?>
        </strong></h2>
      <h2>Disponibles : <strong>
          <?php echo $dis ?>
        </strong></h2>
      <h2>Stock : <strong>
          <?php echo $ps ?>
    ...
    <div class="col-md-2">
      ...
      <form method="POST" ACTION="mostrarpeli.php">
        <?php if ($dinero < $valor) { ?>
        <button type="submit" class="btn btn-danger disabled">Arrendar</button>
        <?php } else { 
          if ($s[0]["stock"]==$s[0]["rentadas"]) { ?>
            <button type="submit" class="btn btn-danger disabled">Arrendar</button>
          <?php }
          else { ?>
      ...
      <form method="POST" ACTION="mostrarpeli.php">
        <button type="submit" name="agregar" value="<?php echo $id ?>" class="btn btn-lg btn-primary">Agregar a
          Favoritas</button>
      </form>
      </br>
      <?php } else { ?>
      <form method="POST" ACTION="mostrarpeli.php">
        <button type="submit" name="eliminar" value="<?php echo $id ?>" class="btn btn-lg btn-primary">Eliminar de
          Favoritas</button>
      </form>
      </br>
      <?php } ?>
...
<div class="card text-white bg-secondary mb-3" style="max-width: 80rem;">
  <div class="row">
    ...
    <div class="col-md-10">
      ...
      <div class="card-body">
        <p class="card-text">
          <?php echo $listaP[0]["descripcionReseña"]; ?>
        </p>
      </div>
    </div>
  </div>
...
$query = $conexion->prepare("SELECT * FROM reseña WHERE idPelicula='$id' and idUsuario!='$IDUSUARIO' ORDER BY fecha DESC ;");
$query->execute();
$listaP = $query->fetchAll(PDO::FETCH_ASSOC);
foreach ($listaP as $reseñas) {
  $x = $reseñas["idUsuario"];
  $res = $reseñas["puntuacion"];
  if ($res > 5) {
    $res = "";
  }
...
<div class="card text-white bg-secondary mb-3" style="max-width: 80rem;">
  <div class="row">
    <div class="col-md-2">
      <div class="card-body">
        <form method="POST" ACTION="otroperfil.php">
          <button type="submit" name="eleccion" value="<?php echo $Usuarios[0]["idUsuario"] ?>"
    ...
    <div class="col-md-10">
      ...
      <div class="card-body">
        <p class="card-text">
          <?php echo $reseñas["descripcionReseña"]; ?>
        </p>
      </div>
    </div>
  </div>
